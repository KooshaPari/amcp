# Root Files Audit: Move to Project Homes
**Date:** 2025-11-22  
**Status:** AUDIT COMPLETE  
**Action:** Move 100+ orphaned files to their respective project folders

---

## 📊 Summary

Files in the root directory that belong to **specific projects** within the monorepo:

| Project | Current Location | Target | Files to Move | Status |
|---------|------------------|--------|---------------|--------|
| **pheno-sdk** | root | `/pheno-sdk/` | 35-40 | Analysis phase |
| **zen-mcp-server** | root | `/zen-mcp-server/` | 15-20 | Analysis phase |
| **router** | root | `/router/` | 20-25 | Analysis phase |
| **bloc** | root | `/bloc/` | 8-10 | Analysis phase |
| **crun** | root | `/crun/` | 5-8 | Analysis phase |
| **4sgm** | root | `/4sgm/` | 3-5 | Analysis phase |
| **Archive** | root | `/archive/` | 10-15 | Analysis phase |
...
DEVELOPING.md (or DEVELOPMENT.md)
DOCSTRING_ADDITIONS_SUMMARY.md
...
Zen MCP Server is the enterprise MCP server. These belong there:

```
→ /zen-mcp-server/docs/research/
...
**Total: ~10 files** → `/zen-mcp-server/docs/research/`

---

## 🎯 ROUTER Files (Move to `/router/`)

Router is the routing engine. These belong there:

```
→ /router/docs/

(Router already has extensive documentation)
Most ROUTER_* files are already there
Look for any *ROUTER* files in root and move them
```
...
(Bloc already has migration docs)
COCOMO_GUIDE.md
...
## 🎯 General Project Files (Root-level)
...
**Total: ~15 files** → `/docs/planning/` (if cross-project) OR root (if orchestration docs)
...
**Total: ~14 files** → `/docs/research/proposals/`
...
**Total: ~2 files** → `/docs/research/mcp-registry/`
...
**Note: Some may be pheno-sdk specific - check file content**
...
Files marked COMPLETE that may be outdated:

```
Destination: /archive/

(Review these first - some may still be needed)
```
...
### Step 1: Identify Project-Specific Files

For each root file, check:
1. **Does the file mention a specific project name?** (pheno, zen-mcp, router, bloc, crun, 4sgm)
2. **Is it a phase report?** → Could be pheno-sdk or cross-project
3. **Is it research/proposals?** → Cross-project or specific project?
4. **Content analysis** - Read first 50 lines to determine ownership
...
### Step 3: Categorize by Reading Headers

For each root file:
1. Open and read first 100 lines
2. Match patterns: "pheno", "zen-mcp", "router", "bloc", "crun", "4sgm"
3. Check git history for context
4. Categorize into appropriate bucket

### Step 4: Execute Moves

Once categorized, move files:
```bash
# Example:
mv /pheno-sdk_file.md /pheno-sdk/docs/
mv /zen-mcp_file.md /zen-mcp-server/docs/research/
mv /general_file.md /docs/
```
...
- Move zen-mcp-server docs to /zen-mcp-server/docs/research/
...
- [ ] Files categorized correctly (by reading headers)
...
- Configuration (`pyproject.toml`, `mkdocs.yml`, `.env`)
- GitHub workflows (`.github/`)
- Utilities and scripts at repo level
- General governance docs (`CLAUDE.md`, `AGENTS.md`)

**All project-specific documentation should live in their projects**, making each project self-contained and discoverable.
